...
inputDir='/eos/uscms/store/user/lpcljm/FWLJMET_crab_1lep2017_UL_fixed'
#inputDir='/isilon/hadoop/store/group/bruxljm/FWLJMET102X_1lep'+str(Year)+'_Oct2019/' # or 2018
outputDir='/eos/uscms/store/user/lpcbril/MC_test/FWLJMET106X_1lep2017_UL_step1/'#'/eos/uscms/store/user/lpcbril/MC_test/FWLJMET102X_1lep'+str(Year)+'_Oct2019_4t_080420_step1_new_Pt40_Eta2p4/nominal/'#'/eos/uscms/store/user/ssagir/FWLJMET102X_1lep'+str(Year)+'_Oct2019_4t_080420_step1/nominal/' # or 2018
condorDir='condor_singleLepton_UL17/'#'/uscms_data/d3/jluo/work/CMSSW_dir/CMSSW_10_2_10/src/LJMet-Slimmer-4tops/step1/condor/'#'/uscms_data/d3/ssagir/FWLJMET102X_1lep'+str(Year)+'_Oct2019_4t_080420_step1/' # or 2018
shifts = []#['JECup','JECdown','JERup','JERdown']
nFilesPerJob=20
inputLoc='lpc'
if inputDir.startswith('/isilon/hadoop/'): inputLoc='brux'

csvFilename='DeepCSV_106XUL17SF.csv'
djcsvFilename='DeepJet_106XUL17SF.csv'
if Year==2018: 
...
dirList = dirList17[:]
#dirList = ['TTToSemiLeptonic_TuneCP5_PSweights_13TeV-powheg-pythia8',
#'TTToSemiLeptonic_TuneCP5_erdON_13TeV-powheg-pythia8',]
if Year==2018: 
...
for sample in dirList:
    ...
    for outlabel in outList:
        ...
        if inputLoc=='lpc':
        	runlist = EOSlistdir(inputDir+'/'+sample+'/')
        elif inputLoc=='brux':
        ...
        for run in runlist:
            jobstring=""
            if inputLoc=='lpc':
                jobstring = EOSlistdir(inputDir+'/'+sample+'/'+run+'/')[0]
            	numlist = EOSlistdir(inputDir+'/'+sample+'/'+run+'/'+jobstring+'/')
            elif inputLoc=='brux':
            	status, dirList = xrdClient.dirlist(inDir+'/'+sample+'/'+finalStateYear+'/'+run+'/')
            	numlist = [item.name for item in dirList]
            
            for num in numlist:
                numpath = inputDir+'/'+sample+'/'+run+'/'+jobstring+'/'+num
                ...
                if inputLoc=='lpc':
                	...
                	status, fileList = xrdClient.dirlist(inDir+'/'+sample+'/'+run+'/'+jobstring+'/'+num+'/')
                ...
                for i in range(0,len(rootfiles),nFilesPerJob):
                    ...
                    dict={'RUNDIR':runDir, 'SAMPLE':sample, 'INPATHSUFFIX':pathsuffix, 'INPUTDIR':inDir, 'FILENAME':basefilename, 'OUTFILENAME':outsample, 'OUTPUTDIR':outDir, 'LIST':idlist, 'ID':tmpcount, 'YEAR':Year, 'CSVFILE':csvFilename, 'DJCSVFILE':djcsvFilename}
...
Transfer_Input_Files = %(RUNDIR)s/compileStep1.C, %(RUNDIR)s/makeStep1.C, %(RUNDIR)s/step1.cc, %(RUNDIR)s/step1.h, %(RUNDIR)s/HardcodedConditions.cc, %(RUNDIR)s/HardcodedConditions.h, %(RUNDIR)s/BTagCalibForLJMet.cpp, %(RUNDIR)s/BTagCalibForLJMet.h, %(RUNDIR)s/%(CSVFILE)s, %(RUNDIR)s/%(DJCSVFILE)s
