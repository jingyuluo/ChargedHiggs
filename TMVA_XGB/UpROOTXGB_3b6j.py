...
sig_selected = (sig_df["NJets_JetSubCalc"]>5)&(sig_df["NJetsCSV_JetSubCalc"]>2)&( ((sig_df["leptonPt_MultiLepCalc"]>35)&(sig_df["isElectron"]==True))|((sig_df["leptonPt_MultiLepCalc"]>30)&(sig_df["isMuon"]==True))) 
...
for ibkg in bkgList:
    ...
    print bkg_df
    bkg_selected = (bkg_df["NJets_JetSubCalc"]>5)&(bkg_df["NJetsCSV_JetSubCalc"]>2)&( ((bkg_df["leptonPt_MultiLepCalc"]>35)&(bkg_df["isElectron"]==True))|((bkg_df["leptonPt_MultiLepCalc"]>30)&(bkg_df["isMuon"]==True)))
...
bst.dump_model('dump.raw'+str(num_round)+'iterations'+"_"+str(param['max_depth'])+'depths_'+str(param['eta'])+'signal_region_'+str(args.varListKey)+'_'+args.label+'_M'+args.mass+'3b6j.txt')
bst.save_model('XGB'+str(num_round)+'iterations'+"_"+str(param['max_depth'])+'depths_'+str(param['eta'])+'signal_region_'+str(args.varListKey)+'_'+args.label+'_M'+args.mass+'3b6j.model')
#print dfall_var

dfall_var.to_root("test_XGB_classification_"+str(args.varListKey)+"_"+'iterations'+"_"+str(param['max_depth'])+'depths_'+str(param['eta'])+args.label+'_M'+args.mass+"3b6j.root", key="XGB_Tree")
