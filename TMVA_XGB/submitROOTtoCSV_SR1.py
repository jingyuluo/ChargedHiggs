...
outputDir= '/isilon/hadoop/users/jluo48/CHiggs/UL17/XGB_CSV_REDNEW/SR1'#'/eos/uscms/store/user/lpcbril/MC_test/FWLJMET106X_1lep2017_UL_step2_CSV_SR1_added/'
...
for mass in Masses:
    ...
    jdf.write(
"""universe = vanilla
Executable = submitROOTtoCSV_SR1.sh
Request_memory = 8000
...
Queue 1"""%dict)
    jdf.close()
    os.system('cp UpROOTtoCSV_SR1.py '+subdir)
    os.system('cp varsList.py '+subdir)
    os.system('cp submitROOTtoCSV_SR1.sh '+subdir)
    os.chdir('%s/'%(subdir))
