...
outputDir= '/isilon/hadoop/users/jluo48/CHiggs/UL17/XGB_CSV_REDNEW/SR2'#'/eos/uscms/store/user/lpcbril/MC_test/FWLJMET106X_1lep2017_UL_step2_CSV_SR2_added/'
condorDir= runDir+'/condor_logs_CSV_SR2/'
...
for mass in Masses:
    ...
    jdf.write(
"""universe = vanilla
Executable = submitROOTtoCSV_SR2.sh
Request_memory = 8000
Should_Transfer_Files = IF_NEEDED
WhenToTransferOutput = ON_EXIT
Transfer_Input_Files = %(RUNDIR)s/UpROOTtoCSV_SR2.py, %(RUNDIR)s/varsList.py 
Output = M%(MASS)s.out
Error = M%(MASS)s.err
Log = M%(MASS)s.log
Notification = Never
...
Queue 1"""%dict)
    jdf.close()
    os.system('cp UpROOTtoCSV_SR2.py '+subdir)
    os.system('cp varsList.py '+subdir)
    os.system('cp submitROOTtoCSV_SR2.sh '+subdir)
    os.chdir('%s/'%(subdir))
