...
outputDir= '/isilon/hadoop/users/jluo48/CHiggs/UL17/XGB_CSV_REDNEW/SRAll'#'/eos/uscms/store/user/lpcbril/MC_test/FWLJMET106X_1lep2017_UL_step2_CSV_SR1_added/'
condorDir= runDir+'/condor_logs_CSV_SRAll/'
...
for mass in Masses:
    ...
    jdf.write(
"""universe = vanilla
Executable = submitROOTtoCSV.sh
Request_memory = 8000
Should_Transfer_Files = IF_NEEDED
WhenToTransferOutput = ON_EXIT
Transfer_Input_Files = %(RUNDIR)s/UpROOTtoCSV.py, %(RUNDIR)s/varsList.py 
Output = M%(MASS)s.out
Error = M%(MASS)s.err
Log = M%(MASS)s.log
Notification = Never
...
Queue 1"""%dict)
    jdf.close()
    os.system('cp UpROOTtoCSV.py '+subdir)
    os.system('cp varsList.py '+subdir)
    os.system('cp submitROOTtoCSV.sh '+subdir)
    os.chdir('%s/'%(subdir))
