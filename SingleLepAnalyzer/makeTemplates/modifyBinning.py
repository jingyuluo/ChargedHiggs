...
templateDir = sys.argv[2]#'templates_M500_2023_5_14_topPtRW_NOHTWeight_Full_FixTrig_forlimit'#'templates_M500_2023_5_1_topPtRW_NOHTWeight_Full_FixTrig_forlimit'#'kinematics_CR_M500_2023_4_22_topPtRW_NC'#'templates_M500_2021_4_25_topPtRW_allweights_UL17_Reshape_ReNorm2D_HTnj_WJetsHTbinned_HTonly'
...
for iPlot in iPlotList:
    ...
    stat = 0.10 #statistical uncertainty requirement (enter >1.0 for no rebinning; i.g., "1.1")
    ...
    totNbins = 0
    xbinsListTemp = {}
    for chn in list(totBkgHists.keys()):
    ...
    print("//"*40)
    xbinsList = {}
    for chn in list(xbinsListTemp.keys()):
        xbinsList[chn] = []
    ...
    xbins = {}
    for key in list(xbinsList.keys()): xbins[key] = array('d', xbinsList[key])
    ...
    for rfile in rfiles: 
        ...
        for chn in channels:
            ...
            for hist in allhists[chn]:
                ...
                if any([item in hist and not removalKeys[item] for item in list(removalKeys.keys())]): continue
            ...
            if doPDF:
                ...
                for hist in pdfUphists:
                    ...
                    for ibin in range(1,pdfUpHist.GetNbinsX()+1):
                        ...
                        indPDFUp = sorted(list(range(len(weightList))), key=lambda k: weightList[k])[83]
                        indPDFDn = sorted(list(range(len(weightList))), key=lambda k: weightList[k])[15]
    ...
    print("        ",sorted([hist[hist.find(bkgProcList[0]+'__')+len(bkgProcList[0])+2:hist.find(upTag)] for hist in list(yieldsAll.keys()) if channels[0] in hist and '__'+bkgProcList[0]+'__' in hist and upTag in hist]))# and 'muRF' not in hist
    print("        following will be removed from yield errors:",sorted(removeSystFromYields))
    if doSmoothing: sorted([hist[hist.find(bkgProcList[0]+'__')+len(bkgProcList[0])+2:hist.find(upTag)] for hist in list(yieldsAll.keys()) if channels[0] in hist and '__'+bkgProcList[0]+'__' in hist and upTag in hist and smoothingAlgo not in hist])
    else: print()
    
    
    def getShapeSystUnc(proc,chn):
        if not addShapes: return 0
        systematicList = sorted([hist[hist.find(proc+'__')+len(proc)+2:hist.find(upTag)] for hist in list(yieldsAll.keys()) if chn in hist and '__'+proc+'__' in hist and upTag in hist])# and 'muRF' not in hist
        totUpShiftPrctg=0
        totDnShiftPrctg=0
        ...
        for nttag in nttaglist:
            for nWtag in nWtaglist:
                for nbtag in nbtaglist:
                    table.append(['break'])
                    table.append(['',isEM+'_'+nttag+'_'+nWtag+'_'+nbtag+'_yields'])
                    table.append(['break'])
                    table.append(['YIELDS']+[chn for chn in channels if isEM in chn and nttag+'_' in chn and nWtag+'_' in chn and nbtag+'_' in chn]+['\\\\'])
                    for proc in bkgProcList+['totBkg',dataName,'dataOverBkg']+sigProcList:
                        row = [procNames[proc]]
                        for chn in channels:
                            if not (isEM in chn and nttag+'_' in chn and nWtag+'_' in chn and nbtag+'_' in chn): continue
                            modTag = chn[chn.find('nW'):]
                            histoPrefix = allhists[chn][0][:allhists[chn][0].find('__')+2]
                            yieldtemp = 0.
                            yielderrtemp = 0.
                    ...
                    for sig in sigProcList:
                        ...
                        for chn in channels: 
                            if not (isEM in chn and nttag+'_' in chn and nWtag+'_' in chn and nbtag+'_' in chn): continue
                            bkgYld_ = float([iList for iList in table[table.index(['',isEM+'_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames['totBkg']][0][iChn].strip().split()[1])
                            bkgYldErr_ = float([iList for iList in table[table.index(['',isEM+'_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames['totBkg']][0][iChn].strip().split()[3])
                            sigYld_ = float([iList for iList in table[table.index(['',isEM+'_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames[sig]][0][iChn].strip().split()[1])
                            sigYldErr_ = float([iList for iList in table[table.index(['',isEM+'_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames[sig]][0][iChn].strip().split()[3])
    ...
    for nttag in nttaglist:
        for nWtag in nWtaglist:
            for nbtag in nbtaglist:
                table.append(['break'])
                table.append(['','isL_'+nttag+'_'+nWtag+'_'+nbtag+'_yields'])
                table.append(['break'])
                table.append(['YIELDS']+[chn.replace('isE','isL') for chn in channels if 'isE' in chn and nttag+'_' in chn and nWtag+'_' in chn and nbtag+'_' in chn]+['\\\\'])
                for proc in bkgProcList+['totBkg',dataName,'dataOverBkg']+sigProcList:
                    row = [procNames[proc]]
                    for chn in channels:
                        if not ('isE' in chn and nttag+'_' in chn and nWtag+'_' in chn and nbtag+'_' in chn): continue
                        modTag = chn[chn.find('nW'):]
                ...
                for sig in sigProcList:
                    ...
                    for chn_ in channels: 
                        ...
                        bkgYld_ = float([iList for iList in table[table.index(['','isL_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames['totBkg']][0][iChn].strip().split()[1])
                        bkgYldErr_ = float([iList for iList in table[table.index(['','isL_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames['totBkg']][0][iChn].strip().split()[3])
                        sigYld_ = float([iList for iList in table[table.index(['','isL_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames[sig]][0][iChn].strip().split()[1])
                        sigYldErr_ = float([iList for iList in table[table.index(['','isL_'+nttag+'_'+nWtag+'_'+nbtag+'_yields']):] if iList[0] == procNames[sig]][0][iChn].strip().split()[3])
                        row.append(' & '+str(round_sig(sigYld_/bkgYldErr_,5)))
                        iChn+=1
                    row.append('\\\\')
                    table.append(row)
    ...
    for proc in bkgProcList+sigProcList:
        ...
        systematicList = sorted([hist[hist.find(proc+'__')+len(proc)+2:hist.find(upTag)] for hist in list(yieldsAll.keys()) if channels[0] in hist and '__'+proc+'__' in hist and upTag in hist])
        for syst in systematicList:
            for ud in [upTag,downTag]:
                row = [syst+ud]
                for chn in channels:
    ...
    for signal in sigProcList:
        ...
        for isEM in isEMlist:        
            for proc in bkgProcList+[dataName,signal]:
                yldHists = {}
                print("lengths", len(channels)/2)
                yldHists[isEM+proc]=TH1F(iPlot+'YLD_'+lumiStr+'_'+isEM+'_nHOT0p_nT0p_nW0p_nB0p_nJ0p__'+proc.replace(signal,'sig').replace('data_obs','DATA'),'',int(len(channels)/2),0,len(channels)/2)
                systematicList = sorted([hist[hist.find(proc)+len(proc)+2:hist.find(upTag)] for hist in list(yieldsAll.keys()) if channels[0] in hist and '__'+proc+'__' in hist and upTag in hist])
                for syst in systematicList:
                    for ud in ['__plus','__minus']: yldHists[isEM+proc+syst+ud]=TH1F(iPlot+'YLD_'+lumiStr+'_'+isEM+'_nHOT0p_nT0p_nW0p_nB0p_nJ0p__'+proc.replace(signal,'sig').replace('data_obs','DATA')+'__'+syst+ud,'',int(len(channels)/2),0,len(channels)/2)
                ibin = 1
                for chn in channels:
